...
// In production CSS RTL is prefixed and compressed
// function cssRtl () {
//   return gulp.src('src/assets/scss/app.rtl.scss')
...
//     .pipe(gulp.src(PATHS.additionalCssFiles2Copy, { since: gulp.lastRun(cssRtl) }))
...
// Compile JS and transform with Babel
// In production JS Rtl is compressed
// function jsRtl () {
//   const bundler = browserify('src/assets/js/app.rtl.js', { debug: true }).transform(babel)
//   return bundler.bundle()
//     .on('error', function (err) { console.error(err.message); this.emit('end') })
//     .pipe(source('app.rtl.js'))
//     .pipe(buffer())
//     .pipe(sourcemaps.init({ loadMaps: true }))
//     .pipe(gulpif(!PRODUCTION, sourcemaps.write('.')))
//     .pipe(gulpif(PRODUCTION, uglify()))
